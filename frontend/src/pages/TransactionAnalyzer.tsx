...
export const TransactionAnalyzer: React.FC = () => {
  ...
  const handleGenerateDemo = async (scenario: "normal" | "suspicious" | "high_risk") => {
    ...
    try {
      ...
      setRawTxn(data);
      setEngineeredFeatures(data.engineered_features || null);
    } catch (e) {
  ...
  const handleAnalyzeDemo = async () => {
    if (!rawTxn || !engineeredFeatures) return;
    setIsLoadingPrediction(true);
    try {
      // Demo-generated transactions carry the exact feature vector of a real
      // dataset row already confirmed (by the live model) to sit in this
      // scenario's risk tier — analyze with those, so the result always
      // matches the scenario button that was clicked. Live per-user feature
      // engineering can't reproduce this reliably: a freshly-picked demo
      // user has no transaction history, so most engineered signals
      // (moving average deviation, velocity counts, etc.) come out as 0
      ...
      <div className="flex border-b border-dark-border">
        <button
          ...
          className={`px-5 py-3 text-sm font-semibold border-b-2 transition ${
            activeTab === "simulate" 
              ? "border-guard-orange text-dark-text" 
            ...
            {rawTxn && (
              <div className="bg-dark-card border border-dark-border rounded-xl p-5 shadow-glow-brand space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    <label className="text-xs text-dark-muted block">Transaction Amount (NGN)</label>
                    <input 
                      type="number" 
                      value={rawTxn.amount} 
                  ...
                  <div>
                    <label className="text-xs text-dark-muted block">Beneficiary Account / USSD ID</label>
                    <input 
                      type="text" 
                  ...
                  <div>
                    ...
                    <select 
                      value={rawTxn.payment_method || "USSD"} 
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {predictionResult.triggered_rules && predictionResult.triggered_rules.length > 0 ? (
                      predictionResult.triggered_rules.map((rule: TriggeredRule, idx: number) => (
                        <div 
                          key={idx} 
                ...
                return (
                  <div key={key} className="space-y-1 relative group">
                    <label className="text-xs font-mono text-dark-text flex items-center gap-1">
                      ...
                      {explanations[key] && (
                        <HelpCircle
                          className="h-3 w-3 text-dark-muted hover:text-dark-text cursor-help"
                          aria-label={explanations[key]}
                        />
                      )}
                    </label>

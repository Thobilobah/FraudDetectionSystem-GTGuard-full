...
export const SystemStatus: React.FC<SystemStatusProps> = ({ health }) => {
  const isRazorpayConfigured = health?.razorpay_mode === "LIVE";
  ...
  const endpoints = [
    ...
    { method: "POST", path: "/webhooks/razorpay", desc: "Live integration endpoint validating HMAC capture events" },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-dark-card border border-dark-border rounded-xl p-5 shadow-glow-brand md:col-span-1 space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between p-3 bg-gray-50 border border-dark-border rounded-lg text-xs">
              <span className="text-dark-muted">SQLite Repository</span>
            ...
            <div className="flex items-center justify-between p-3 bg-gray-50 border border-dark-border rounded-lg text-xs">
              <span className="text-dark-muted">Razorpay Integration</span>
        ...
        <div className="bg-dark-card border border-dark-border rounded-xl p-5 shadow-glow-brand md:col-span-2 flex flex-col justify-between">
          <div className="space-y-3">
            <h3 className="text-base font-semibold text-dark-text">Razorpay Webhook Credentials</h3>
            {isRazorpayConfigured ? (
              <div className="bg-brand-success/5 border border-brand-success/20 rounded-xl p-4 text-xs space-y-2 text-brand-success leading-relaxed">
                ...
                <p className="text-dark-text">
                  The API is listening for capturing webhook requests on `POST /webhooks/razorpay`.
                  Signatures will be verified securely using SHA256 HMAC encryption keys.
                </p>
              </div>
            ) : (
              <div className="bg-brand-warning/5 border border-brand-warning/20 rounded-xl p-4 text-xs space-y-2 text-brand-warning leading-relaxed">
                <p className="font-bold flex items-center gap-1.5">
                  <AlertTriangle className="h-4 w-4 shrink-0" /> Razorpay integration not configured — running in Demo Mode.
                </p>
                <p className="text-dark-text">
                  No live secrets (`RAZORPAY_KEY_ID`, `RAZORPAY_WEBHOOK_SECRET`) were detected in `.env`.
          ...
          <div className="space-y-2 mt-3">
            ...
            <div className="text-xs text-brand-primary font-mono bg-brand-primary/5 border border-brand-primary/15 p-2.5 rounded-lg">
              <span className="text-[9px] text-brand-primary block uppercase font-bold mb-1 tracking-wider">Public Tunnel Endpoint (Serveo)</span>
              https://[your-tunnel-id].serveousercontent.com/webhooks/razorpay
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-dark-card border border-dark-border rounded-xl p-5 shadow-glow-brand space-y-4">
        ...
        <p className="text-xs text-dark-muted leading-relaxed">
          Razorpay provides simple raw transaction objects. It will <strong>NOT</strong> provide high-dimensional fraud signals.
          Our FraudGuard feature-engineering layer enriches every raw payment capture event in real-time, executing SQL queries,
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3 pt-2 text-center text-xs">
          <div className="bg-gray-50/50 border border-dark-border rounded-lg p-3 space-y-1">
            ...
            <span className="text-dark-text block font-semibold font-mono">Amount, VPA, GPS, Time</span>
          </div>
          ...
          <div className="bg-gray-50/50 border border-dark-border rounded-lg p-3 space-y-1 md:col-span-2">
            <span className="text-[10px] text-brand-primary font-bold block">2. FEATURE ENGINEERING ENGINE</span>
            ...
            <div key={idx} className="p-4 flex flex-col sm:flex-row items-start gap-3 sm:gap-6 text-sm">
              <span className={`px-3 py-1 rounded text-xs font-bold font-mono min-w-[70px] text-center ${
                ep.method === "GET" 
                  ...
                  : "bg-brand-primary/10 text-brand-primary border border-gray-200"
              }`}>
                {ep.method}
              </span>
